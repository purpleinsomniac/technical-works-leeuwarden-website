...
<title>Technical Works Leeuwarden</title>
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  /* corner brackets, echoes technical drawing annotation */
  ...
  .nav-cta{
    ...
    background:var(--ink); color:var(--white); padding:11px 20px;
  ...
  .hero p{
    max-width:520px; margin-top:26px; font-size:17px; color:#3C4550;
  }
  .hero-actions{display:flex; gap:16px; margin-top:40px; flex-wrap:wrap;}
  .btn-primary{
    background:var(--blue); color:var(--white); padding:15px 28px;
    font-size:13px; font-weight:600; text-transform:uppercase; letter-spacing:.06em;
    border:1px solid var(--blue); transition:background .2s ease;
  }
  .btn-primary:hover{background:#164680;}
  .btn-secondary{
    padding:15px 28px; font-size:13px; font-weight:600; text-transform:uppercase; letter-spacing:.06em;
    border:1px solid var(--ink); transition:all .2s ease;
  }
  .btn-secondary:hover{background:var(--ink); color:var(--white);}

  /* signature: animated stroke that draws the T, then the W, of the logo */
  .zig-wrap{
    position:absolute; right:-20px; top:120px; width:480px; height:340px; z-index:1;
    opacity:0.95;
  }
  .zig-wrap svg{width:100%; height:100%;}
  .zig-path{fill:none; stroke-linecap:square; stroke-linejoin:miter;}
  .zig-path.t{
    stroke:var(--ink); stroke-width:5; opacity:.55;
    stroke-dasharray:420; stroke-dashoffset:420;
    animation:draw 1s ease forwards .3s;
  }
  .zig-path.w{
    stroke:var(--blue-light); stroke-width:4;
    stroke-dasharray:1050; stroke-dashoffset:1050;
    animation:draw 1.5s ease forwards 1.15s;
  }
  ...
  .tag{
    font-size:12px; color:var(--blue); margin-bottom:14px; display:block;
  }
  ...
  .service-card{
    background:var(--bg); padding:38px; position:relative;
  }
  .service-card .idx{
    font-size:12px; color:var(--steel);
  }
  .service-card h3{
    font-size:22px; margin:16px 0 12px; color:var(--ink);
  }
  ...
  @media (max-width:820px){
    ...
    .zig-wrap{display:none;}
    .contact .wrap{align-items:flex-start;}
  }
  @media (prefers-reduced-motion: reduce){
    .zig-path{animation:none; stroke-dashoffset:0;}
...
<section id="diensten">
  <div class="wrap">
    <div class="section-head reveal">
      ...
      <p>Van opmaak tot database. Vier vaste bouwstenen, één aanspreekpunt.</p>
    </div>
    <div class="services-grid reveal">
      <div class="service-card bracket">
        <span class="idx mono">01 / HTML</span>
        <h3>HTML</h3>
        <p>De structuur van de website: heldere, toegankelijke opbouw waar alles op voortbouwt.</p>
      </div>
      <div class="service-card bracket">
        <span class="idx mono">02 / CSS</span>
        <h3>CSS</h3>
        <p>De vormgeving en opmaak: strak, responsief en passend bij de huisstijl.</p>
      </div>
      <div class="service-card bracket">
        <span class="idx mono">03 / PHP</span>
        <h3>PHP</h3>
        <p>De logica achter de website: formulieren, gebruikersacties en dynamische pagina's.</p>
      </div>
      <div class="service-card bracket">
        <span class="idx mono">04 / MYSQL</span>
        <h3>MySQL</h3>
        <p>De database: gegevens opslaan, ophalen en overzichtelijk beheren.</p>
      </div>
    </div>
  </div>
</section>
...
<footer>
  <div class="wrap">
    <span>© 2026 Technical Works Leeuwarden</span>
